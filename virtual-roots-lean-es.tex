...
\medskip
\noindent El desarrollo son dos ficheros Lean: \lean{VirtualRoots.lean} para la teor\'ia estructural, y
\lean{VirtualRootsCount.lean} para el recuento exacto construido encima. Los lemas que soportan el peso
est\'an reunidos en la Secci\'on~\ref{sec:blocks}, y los l\'imites que quedan---la continuidad, y el
objeto global en todo $\R$---en la Secci\'on~\ref{sec:boundary}. Si solo lees dos cosas, lee el
entrelazado (Teorema~\ref{thm:interlace}) y el recuento (Teorema~\ref{thm:count}).
...
t\'ermino. Sumar los escalones da el Teorema~\ref{thm:count}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lc@{}}
    ...
    Teoremas estructurales / defs (\lean{VirtualRoots}) & $16\,/\,2$ \\
    \rowcolor{ggband}
    Recuento: importa \lean{VirtualRoots} y \lean{BudanFourier} & dos recursiones fundidas \\
...
\item \emph{Dos definiciones de un objeto.} La igualdad principal
$\#\{\text{ra\'ices virtuales en }(a,b]\}=\V(a)-\V(b)$ ya est\'a probada (Secci\'on~\ref{sec:count}) por
la primera de dos rutas honestas: \emph{puentea} la construcci\'on de aqu\'i---el minimizador
$\Rd_d$---con la funci\'on de variaci\'on de signos \lean{fourierVar} del trabajo
compa\~nero~\cite{bflean}, convirtiendo la desigualdad de aquel paper~\cite{coste,glm} en igualdad. Hay
una segunda ruta, no tomada: \emph{definir} las ra\'ices virtuales una segunda vez, como los saltos de
\lean{fourierVar}, donde el recuento es casi por definici\'on, y probar que ambas definiciones
coinciden. El entrelazado fue gratis en la primera definici\'on; el recuento ser\'ia gratis en la
segunda; \textbf{la equivalencia de ambas definiciones es el teorema que esa ruta aislar\'ia}, y cu\'al
es m\'as corta en Lean sigue siendo la pregunta.
