...
recurrence, for which I could find no prior formalization in any proof
...
was a prerequisite, and I could find no prior formalization of it in any proof
assistant.\footnote{I searched for a prior formalization of the matching
polynomial across the major proof assistants (June~2026): Mathlib (Lean), the
Archive of Formal Proofs (Isabelle), the Coq/MathComp ecosystem, HOL~Light, and
Agda. \emph{Matchings} as edge sets are formalized---for instance
maximum-cardinality and stable matching in the AFP, and Hall's theorem in Lean
and Coq---but I found no formalization of the matching \emph{polynomial} $\mu_G$,
its coefficients $m_k$, or its deletion recurrence. The search was best-effort,
not exhaustive; hence ``to my knowledge''.}
...
The Marcus--Spielman--Srivastava theorem on Ramanujan graphs of every degree is
celebrated, and no proof assistant has, to my knowledge, yet touched it. This paper
...
matching polynomial is now available in a prover where I could find no prior
formalization of it, and that the shared engine is visible in the dependency
graph rather than asserted. The deep half
...
\paragraph{Artifacts.} All Lean sources (\lean{sorry}-free, depending only on the
three standard axioms; see Table~\ref{tab:results}), the figures of this paper,
and the numerical cross-checks of Table~\ref{tab:checks} are available at:
\begin{center}
\url{https://github.com/karlesmarin/godsil-gutman-lean}\\[2pt]
archived at \textsc{doi}~\href{https://doi.org/10.5281/zenodo.20517350}{10.5281/zenodo.20517350}
\end{center}
