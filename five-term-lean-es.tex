...
$x\cdot y$ a $y\cdot x$, y luego necesita $1-yx\neq0$, en \emph{ese} orden de t\'erminos y ya
en el contexto, para descargar el inverso que produce. El arreglo es una l\'inea ---enunciar
el hecho de no-anulaci\'on para ambos \'ordenes, antes de la simplificaci\'on--- pero hallarlo
cost\'o m\'as que el resto de la prueba.
...
\emph{Formalizaciones previas.} La afirmaci\'on de prioridad descansa en una b\'usqueda
documentada, no en una intuici\'on. El 12--13 de junio de 2026 busqu\'e, para el dilogaritmo y
para la relaci\'on de cinco t\'erminos: Mathlib (Loogle y \lean{grep}; no existe declaraci\'on
\lean{dilog}/\lean{polylog}, solo un comentario que remite a $\Li$) y los repositorios m\'as
amplios de la comunidad Lean; el Archive of Formal Proofs de Isabelle; el ecosistema Coq/Rocq
(\texttt{coq-community}, \texttt{opam}); HOL~Light; y Metamath (\texttt{set.mm}). Ninguno
contiene el dilogaritmo como objeto anal\'itico con sus ecuaciones funcionales, ni la
relaci\'on de cinco t\'erminos. El respaldo estructural es m\'as simple que cualquier
b\'usqueda: la relaci\'on necesita un $\Li$ formalizado para poder \emph{enunciarse}, y
ning\'un asistente de pruebas lo ten\'ia hasta que el paper compa\~nero lo construy\'o ---as\'i
que un cinco t\'erminos verificado por m\'aquina no puede ser anterior a eso. Como siempre,
tal afirmaci\'on se hace hasta donde uno sabe.

\emph{Tama\~no de la formalizaci\'on.} El trabajo nuevo de este paper es
\lean{Dilog/FiveTerm.lean}: $243$ l\'ineas, cinco teoremas (Tabla~\ref{tab:inventory}), que
elaboran en $17$~s una vez cacheados Mathlib y el \lean{Dilog/Basic.lean} del compa\~nero.
Descansa sobre \lean{Dilog/Basic.lean} ($581$ l\'ineas: el dilogaritmo, su derivada,
reflexi\'on, Landen, duplicaci\'on) del paper compa\~nero, y sobre Mathlib para todo el
an\'alisis (derivaci\'on t\'ermino a t\'ermino, la constancia por el valor medio, el l\'imite
$-t\log t$). La prueba reutiliza la biblioteca anal\'itica de Mathlib al completo; lo original
es el puente de $243$ l\'ineas y los tres bloques reutilizables. Cada teorema depende solo de
los tres axiomas est\'andar (\lean{propext}, \lean{Classical.choice}, \lean{Quot.sound}); sin
\lean{native\_decide}, sin \lean{sorry}, sin axioma propio.
