...
was tight. Roughly half of it is not about Sturm at all: the sign-variation theory
(\lean{signChanges}, \lean{wallCount}, \lean{FlankReduce}, and the list-surgery lemmas) is a
free-standing combinatorial layer, reusable for any sign-counting argument---as its \lean{rfl}
identification with Descartes' \lean{signVariations} (Section~\ref{sec:q4}) already shows. The
Sturm-specific remainder is the sequence's structure and the local-to-global crossing analysis.
...
compile time & $\approx 9$\,s (file only, pre-built Mathlib), Lean \lean{v4.30.0-rc2} \\
axioms & \lean{propext}, \lean{Classical.choice}, \lean{Quot.sound} \\
reusable layer & sign-variation theory, $\approx$ half the file, Sturm-independent \\
\bottomrule
\end{tabular}
