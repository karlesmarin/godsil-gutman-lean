...
\begin{tikzpicture}[
  ...
  \node[obj,right=18mm of seq] (count) {a count of\\sign changes};
  \node[obj,right=18mm of count] (quantum) {one root,\\one step (per mult.)};
  \node[obj,below=14mm of quantum] (glue) {glue the steps\\over $(a,b]$};
  \node[obj,left=18mm of glue] (free) {Descartes,\\and its even gap};
  \node[obj,left=18mm of free,fill=ggamber!25,draw=ggamber] (done)
    {\textbf{Budan--Fourier,}\\\textbf{machine-checked}};
  \draw[arr] (seq) -- node[q,above=1mm] {Q1: how can\\subtraction bound?} (count);
  \draw[arr] (count) -- node[q,above=1mm] {Q2: why move,\\and by how much?} (quantum);
...
$p_B$: the drop happens at a critical point that no root explains, and it comes in a block of even
size because $p_B$'s missing roots are a conjugate pair. This is the phenomenon Descartes' rule
exhibits globally; Budan--Fourier localizes it.
...
derivative of the one above it, $p^{(k+1)}=(p^{(k)})'$. So if $p^{(k)}(c)=0$ while $p^{(k+1)}$ keeps a
constant nonzero sign $s$ just off $c$, then $p^{(k)}$ is strictly monotone there, and its sign just
off $c$ is forced---\emph{the same} as $p^{(k+1)}$ on the right, the \emph{opposite} on the left. No
Taylor expansion, no multiplicity bookkeeping: one application of the mean value theorem.
...
then $\mathrm{sign}\,q(z)=-s$ (\lean{sign\_eval\_left\_vanish}). Apply this up a vanishing block, from
the first \emph{non}-vanishing member above it downward: \textbf{on the right of $c$ each vanishing
member copies the sign above it}, so the block collapses to a single sign and contributes \emph{no}
sign change; \textbf{on the left each vanishing member takes the opposite sign}, so the block
\emph{alternates} and contributes one sign change per member.
\end{recipebox}

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{budan_fig2_critical}
\caption{Anatomy of a critical point, for the double root $c=1$ of $p_A$ (the block $p,p'$ vanishes,
$m=2$). Reading each sign just off $c$ from the member above it: on the right (teal) a vanishing
member copies the sign above---the block collapses, $\V(c^+)=0$; on the left (plum) it takes the
opposite sign---the block alternates, $\V(c^-)=2$. Hence $\V(c^-)-\V(c^+)=m$, the multiplicity, plus
...
entry depends on whether \emph{that} member vanishes at $c$ and on the sign of the member above it.
...
each $0$ by the value below it and \lean{Lseq} replaces it by the negation of the value below it.
...
\begin{tikzpicture}[
  ...
  \node[lbl,text=ggplum!80!black,text width=4.6cm] at (5.6,1.5)
    {a vanisher takes the \emph{negated} sign below $\Rightarrow$ the block alternates,
     $+1$ per vanisher: $\V(c^-)=\V(c)+m+2e$};
  \node[lbl,text=ggteal!80!black,text width=4.6cm] at (5.6,-1.5)
    {a vanisher \emph{copies} the sign below $\Rightarrow$ the block collapses, no new change:
     $\V(c^+)=\V(c)$};
...
vanishing entry copies, resp.\ negates, the surviving sign below it---and their counting
...
\emph{Descartes, and why it overcounts evenly.} Descartes' rule is the $b\to\infty$ shadow of
Budan--Fourier: the signs of $p^{(k)}$ at large positive $x$ are the signs of the leading
coefficients, and the sign variations there are exactly the coefficient sign variations Mathlib
already counts. Budan--Fourier on $(0,\infty)$ then says: the number of positive roots is at most the
coefficient sign variations, and differs from it by an even number. That even number is the
content---it is why Descartes' rule, applied to a polynomial with $v$ sign changes, never reports a
parity error: \textbf{the unseen roots come in conjugate pairs, and each pair contributes $2$ to the gap}.
Example B is the smallest instance: $x^2+1$ has $\V=2$ sign variations on the half-line and zero real
roots, the gap a single conjugate pair. The Lean count used here and Mathlib's
...
the even gap. Figure~\ref{fig:shadow} makes the accounting visible: the bound splits into the real
roots actually present and twice the conjugate pairs the interval cannot see.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{budan_fig3_shadow}
...
A string of beads is meant to be continued. I close with the questions I would rather carry forward
than pretend to have settled---each a place where this work stops short, and each, I think, the start
of another paper.
...
\item \emph{One theorem behind two counts.} Descartes' coefficient count and the tower count are, after
unfolding, the same kind of object (Section~\ref{sec:q4}), and Sturm's chain count is built on the same
toolkit (\cite{sturmlean}). Is there a single Lean statement---a sign-variation principle on an abstract
``admissible sequence''---of which Descartes' bound, Budan--Fourier's interval bound, and Sturm's exact
count are three instances, so the library holds the local lemma once rather than three times?
...
\bibitem{liBF} W.~Li, Counting Polynomial Roots in Isabelle/HOL: A Formal Proof of the Budan--Fourier
Theorem, in \emph{Proc.\ 8th ACM SIGPLAN Int.\ Conf.\ on Certified Programs and Proofs (CPP~2019)}, ACM,
2019, 52--64; preprint \texttt{arXiv:1811.11093}; Archive of Formal Proofs entry ``The Budan--Fourier
Theorem and Counting Real Roots with Multiplicity'' (2018),
\url{https://isa-afp.org/entries/Budan_Fourier.html}.
