...
in the order they actually arose; each section opens with its question and ends by
handing the reader the next one. Figure~\ref{fig:map} is the map. If you read only one
thing, read Theorem~\ref{thm:clock} and Figure~\ref{fig:clock}; the Lean inventory is
Table~\ref{tab:inventory}, and reproduction takes three commands
...
\node[obj, right=14mm of hole] (li2) {$\Li$ and its\\functional equations};
\node[obj, right=14mm of li2] (gold) {the golden\\ladder};
\node[obj, below=9mm of gold] (circle) {the circle:\\parabola $+$ $\Cl$};
...
\draw[arr] (hole) -- node[q, above=0.5mm] {Q1: what does the\\series know?} (li2);
\draw[arr] (li2) -- node[q, above=0.5mm] {Q2: ladder without\\Abel?} (gold);
\draw[arr] (gold) -- node[q, right=1mm] {Q3: what happens\\on the circle?} (circle);
\draw[arr] (circle) -- node[q, above=0.5mm] {Q4: why is the\\wild half positive?} (pos);
\draw[arr] (pos) -- node[q, above=0.5mm] {Q5: what physical thing\\beats to this?} (clock);
\end{tikzpicture}
\caption{The journey, question by question. Every node is a block of
\lean{sorry}-free Lean; every arrow is the question that produced the next block.}
\label{fig:map}
\end{figure}
...
Figure~\ref{fig:golden} shows the curve with its certified values.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{dilog_fig1_golden}
\caption{The real dilogarithm. Every marked value is a \lean{sorry}-free Lean theorem;
the generating script asserts each against $40$-digit numerics before drawing.}
\label{fig:golden}
\end{figure}
...
\;=\;0.9159\ldots,
\]
\textbf{Catalan's constant}, absent from every proof assistant until now
...
\emph{The turn --- the big one.} Step back and look at what we are holding. A sum
$\sum p_n e^{-in\theta}$ with positive weights $p_n\propto 1/n^2$, whose imaginary part
we have just proven never vanishes on $(0,\pi)$. A physicist reads that expression
instantly: it is the \emph{autocorrelation of a quantum state} --- the inner product
between a state and its own time evolution, for populations $p_n$ on equally spaced
energy levels. The circle has been measuring \emph{time} all along, and our positivity
theorem is a statement about a quantum system that cannot reach an orthogonal state.
How fast \emph{can} any quantum system reach one?
...
limits of quantum mechanics predict nothing at all. One might guess such an
...
circle: the state never gets even \emph{halfway} to distinguishability
...
$(\lvert0\rangle+\lvert2E\rangle)/\sqrt2$ (dashed) saturates Margolus--Levitin and
...
circle --- and the Enestr\"om--Kakeya theorem (1893/1912)~\cite{enestrom} localizes such
zeros: they occur only in the equality cases, where the decrements $p_{n-1}-p_n$
concentrate on an arithmetic progression, placing the zeros at roots of unity. The
Margolus--Levitin--sharp state $(\lvert0\rangle+\lvert2E\rangle)/\sqrt2$ is exactly such
an equality case ($\frac12(1+z)$, zero at $z=-1$). We have not found this dictionary
--- root location of positive-coefficient polynomials $\leftrightarrow$
orthogonalizability of monotone quantum states --- in either literature; we record it as
an observation and a road.
\end{remark}
...
\lean{Classical.choice}, \lean{Quot.sound}. The mathematics, the mistakes, and the
...
File & Contents & Lines\\
\midrule
\lean{Dilog/Basic.lean} & $\Li$: series, derivative, reflection, Landen,&\\
& duplication, $\Li(\frac12)$, golden ladder, Rogers $\Lrog$, Lee--Yang & 581\\
\lean{Dilog/Clausen.lean} & $\Cl$, Catalan's $G$, Bernoulli parabola, $\Cl\ge\sin\theta/2$,&\\
& $\Cl(2\pi-\theta)=-\Cl(\theta)$, Theorem~\ref{thm:clock} & 332\\
\lean{Dilog/FejerJackson.lean} & Fej\'er--Jackson inequality & 189\\
\lean{QSL/Basic.lean} & Margolus--Levitin (+ the cosine inequality) & 238\\
\lean{QSL/MandelstamTamm.lean} & Mandelstam--Tamm, $L^1$ form & 134\\
\midrule
&& 1\,474\\
\bottomrule
\end{tabular}
