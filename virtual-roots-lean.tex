...
\medskip
\noindent The development is a single Lean file, \lean{VirtualRoots.lean}; its load-bearing lemmas are
gathered in Section~\ref{sec:blocks}, and its limits---chiefly the deferred exact count---are stated
in Section~\ref{sec:boundary}. If you read only two things, read the interlacing
(Theorem~\ref{thm:interlace}) and Figure~\ref{fig:fillin}.
...
The object is due to Gonz\'alez-Vega, Lombardi and Mah\'e~\cite{glm}, and it is the natural carrier of
the Budan--Fourier count, made an equality through it by Coste, Lajous, Lombardi and Roy~\cite{coste}.
This paper formalizes the part of that story that is structural---the construction, the count, the
order, and the Rolle interlacing---and is honest, in Section~\ref{sec:boundary}, about the arithmetic
count it leaves to the sequel. I came to it as I came to Sturm~\cite{sturmlean} and
Budan--Fourier~\cite{bflean}: by asking which classical results in real-root counting a mature library
is still missing, and finding the virtual roots absent from every prover I could search.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lc@{}}
    ...
    \midrule
    \rowcolor{ggband}
    Lines in \lean{VirtualRoots.lean} & $369$ \\
    Public theorems / definitions ($+$ private) & $16\,/\,2$ ($+1$) \\
    \rowcolor{ggband}
    Mathlib dependencies (imports) & $5$ modules \\
    \multicolumn{2}{@{}l@{}}{\quad\footnotesize\lean{Deriv.Polynomial}, \lean{Deriv.MeanValue}, \lean{Topology.Algebra.Polynomial},} \\
    \multicolumn{2}{@{}l@{}}{\quad\footnotesize\lean{Topology.Order.Compact}, \lean{Topology.Order.IntermediateValue}} \\
    \rowcolor{ggband}
...
Three facts this development does not settle mark where it points next; each is a place the work stops
short, and each is the start of the following one.
