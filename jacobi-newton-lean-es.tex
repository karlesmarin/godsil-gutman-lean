...
\emph{con punto base}, no clases geod\'esicas. La contribuci\'on es la formalizaci\'on, y un pu\~nado
de lemas certificados que rellenan piezas que faltaban en el ecosistema de \'algebra lineal de
Mathlib---la f\'ormula algebraica de Jacobi, la identidad de Newton matricial-de-trazas, la serie
resolvente y el conteo de caminos dirigidos.
\end{abstract}
...
enmarcadas; los l\'imites honestos se enuncian con la misma claridad que los resultados.\footnote{\emph{Reproducibilidad.}
Todas las fuentes Lean son p\'ublicas en \url{https://github.com/karlesmarin/godsil-gutman-lean}
(commit \texttt{67d7d0a}), construidas contra Lean \texttt{v4.30.0-rc2} y el commit fijado de Mathlib
\texttt{701fb6e9c3}. Reprod\'uzcase con \texttt{lake exe cache get \&\& lake build}; comprueba la
huella de axiomas de cualquier resultado con \lean{\#print axioms}. Las comprobaciones num\'ericas de
la Tabla~\ref{tab:checks} est\'an en \texttt{verify\_jacobi\_newton.py}.}
...
donde $\adj M$ es la adjugada (la traspuesta de la matriz de cofactores)~\cite{HornJohnson}. Es el
c\'alculo matricial~\cite{MagnusNeudecker} tras la f\'ormula de Liouville para el wronskiano de una EDO
lineal, tras el algoritmo de Faddeev--LeVerrier que computa un polinomio caracter\'istico desde las
trazas~\cite{Hou1998}, y tras toda perturbaci\'on a primer orden de un autovalor. Es tambi\'en, en su forma \emph{algebraica}---sobre un
...
La segunda son las \emph{identidades de Newton}~\cite{Macdonald}. Newton relacion\'o las sumas de potencias
...
Este art\'iculo formaliza esa cadena---Jacobi, luego Newton, luego los conteos de Ihara---en Lean~4~\cite{LeanFour}, y
...
espectral de grafos~\cite{BrouwerHaemers}: tiempos de mezcla, el m\'etodo de los momentos para distribuciones de
...
\paragraph{Por qu\'e es \'util, llanamente.}
Los conteos y espectros no-retroceso sustentan umbrales agudos de epidemias y
percolaci\'on~\cite{KarrerNewmanZdeborova2014}, la detecci\'on espectral de
comunidades~\cite{Krzakala2013} y---a trav\'es del an\'alogo de Ihara de la hip\'otesis de
Riemann---la definici\'on misma de un grafo de Ramanujan como expansor \'optimo~\cite{LPS1988,HLW2006}. El
...
\toprule
\rowcolor{ggteal}
{\color{white}\textbf{Nombre Lean}} & {\color{white}\textbf{Enunciado}} & {\color{white}\textbf{\S}}\\
\midrule
...
\toprule
\rowcolor{ggteal}
{\color{white}\textbf{Identidad}} & {\color{white}\textbf{Instancia}} & {\color{white}\textbf{Resultado}}\\
\midrule
...
asistente de pruebas---busqu\'e en Mathlib, el \emph{Archive of Formal Proofs} de Isabelle/HOL, los
\'indices de Coq/Rocq, HOL~Light y la MML de Mizar---, de la f\'ormula \emph{algebraica} de Jacobi
$(\det M)'=\tr(\adj M\cdot M')$; (N2)
...
Llev\'e a cabo este desarrollo usando un sistema de IA como instrumento de investigaci\'on. Yo fij\'e
los objetivos---qu\'e identidades formalizar, la estrategia de prueba de cada una, la frontera que
afirmar y la frontera que rechazar---y el asistente propuso definiciones, lemas y t\'acticas
candidatas, que un lazo de compilaci\'on-como-or\'aculo aceptaba o rechazaba contra el n\'ucleo. Las
decisiones matem\'aticas, el alcance y cada afirmaci\'on de este art\'iculo son m\'ias y de mi
responsabilidad; la contabilidad cuidadosa que resulta---cada afirmaci\'on dimensionada exactamente a
lo demostrado, la distinci\'on punto-base-frente-a-geod\'esica trazada con la misma nitidez que los
teoremas---es parte de lo que espero que el trabajo muestre: que la formalizaci\'on asistida por IA,
dirigida y auditada por una persona, puede sostenerse en un est\'andar de honestidad \emph{m\'as alto}
que la matem\'atica informal, precisamente porque el asistente de pruebas, no el asistente, tiene la
\'ultima palabra.
...
La matem\'atica es cl\'asica: la f\'ormula de Jacobi y la de Liouville~\cite{HornJohnson,MagnusNeudecker},
las identidades de Newton~\cite{Macdonald} y el algoritmo de Faddeev--LeVerrier construido sobre
ellas~\cite{Hou1998}, la lectura de las potencias de matrices como conteo de
caminos~\cite{BrouwerHaemers} y---para la aplicaci\'on---la funci\'on zeta de Ihara~\cite{Ihara1966},
el operador
...
y de la f\'ormula de Bass~\cite{IharaBass}. Tras buscar en Mathlib, el \emph{Archive of Formal Proofs}
de Isabelle/HOL, Coq/Rocq, HOL~Light y Mizar, no tengo noticia de una demostraci\'on previa
verificada por m\'aquina de la f\'ormula algebraica de Jacobi ni de la identidad de Newton
matricial-de-trazas en ninguno de ellos; lo enuncio como resultado de b\'usqueda, no como teorema, y
agradecer\'ia que se me se\~nale algo que haya pasado por alto.
...
\end{center}
{\sloppy en \lean{Ihara/TraceFormula.lean} (Jacobi, Newton, la composici\'on de Ihara) y
...
$\rightsquigarrow$ \lean{[propext, Classical.choice, Quot.sound]}.\par}

\begin{thebibliography}{99}
\bibitem{Bass1992} H.~Bass, \emph{The Ihara--Selberg zeta function of a tree lattice}, Internat. J.
  Math. \textbf{3} (1992), 717--797.
\bibitem{BrouwerHaemers} A.~E.~Brouwer y W.~H.~Haemers, \emph{Spectra of Graphs}, Universitext,
  Springer, 2012.
\bibitem{HLW2006} S.~Hoory, N.~Linial y A.~Wigderson, \emph{Expander graphs and their applications},
  Bull. Amer. Math. Soc. \textbf{43} (2006), 439--561.
\bibitem{HornJohnson} R.~A.~Horn y C.~R.~Johnson, \emph{Matrix Analysis}, 2.\textsuperscript{a} ed.,
  Cambridge University Press, 2013.
\bibitem{Hou1998} S.-H.~Hou, \emph{Classroom note: a simple proof of the Leverrier--Faddeev
  characteristic polynomial algorithm}, SIAM Rev. \textbf{40} (1998), 706--709.
...
\bibitem{KarrerNewmanZdeborova2014} B.~Karrer, M.~E.~J.~Newman y L.~Zdeborov\'a, \emph{Percolation on
  sparse networks}, Phys. Rev. Lett. \textbf{113} (2014), 208702.
\bibitem{Krzakala2013} F.~Krzakala, C.~Moore, E.~Mossel, J.~Neeman, A.~Sly, L.~Zdeborov\'a y P.~Zhang,
  \emph{Spectral redemption in clustering sparse networks}, Proc. Natl. Acad. Sci. USA \textbf{110}
  (2013), 20935--20940.
\bibitem{LeanFour} L.~de~Moura y S.~Ullrich, \emph{The Lean~4 theorem prover and programming
  language}, en \emph{Automated Deduction---CADE 28}, LNCS \textbf{12699}, Springer, 2021, 625--635.
\bibitem{LPS1988} A.~Lubotzky, R.~Phillips y P.~Sarnak, \emph{Ramanujan graphs}, Combinatorica
  \textbf{8} (1988), 261--277.
\bibitem{Macdonald} I.~G.~Macdonald, \emph{Symmetric Functions and Hall Polynomials},
  2.\textsuperscript{a} ed., Oxford University Press, 1995.
\bibitem{MagnusNeudecker} J.~R.~Magnus y H.~Neudecker, \emph{Matrix Differential Calculus with
  Applications in Statistics and Econometrics}, 3.\textsuperscript{a} ed., Wiley, 2019.
