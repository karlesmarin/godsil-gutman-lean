...
\begin{abstract}
Este es un companion de \emph{De signos aleatorios a emparejamientos}~\cite{PartI} y
...
Figura~\ref{fig:nb}---la identidad, y la imagen de por qu\'e un determinante de tama\~no-arista es en
realidad de tama\~no-v\'ertice. La demostraci\'on es un solo viaje: un c\'alculo de incidencia de
...
$\zeta_G(u)^{-1}=\det(I_{2|E|}-uB)$, y el teorema de Bass lo reduce a tama\~no-v\'ertice.
...
\paragraph{La ruta, de un respiro.}
...
de mejor esfuerzo en Lean/Mathlib, el Archive of Formal Proofs (Isabelle) y el ecosistema
...
que el colapso no es un milagro: es la afirmaci\'on de que el operador de aristas porta solo $|V|$
v\'ertices de informaci\'on espectral genuina, envuelta en una involuci\'on. La f\'ormula de Bass es
...
\begin{quote}\itshape
  ¿por qu\'e habr\'ia el \emph{mismo} n\'umero, $2\sqrt{\Delta-1}$, de acotar las ra\'ices del
  polinomio de emparejamientos (Paper~II) y de marcar la recta cr\'itica de la zeta de Ihara---dos
  objetos construidos a partir de $G$ de formas en apariencia inconexas?
\end{quote}
...
Lean, y la f\'ormula de traza misma ($p_k=\operatorname{tr}(A^k)-N_k$, exacta hasta el cuello) se
...
Este desarrollo es el ``lado Ihara'' companion de mis formalizaciones previas en Lean del polinomio
