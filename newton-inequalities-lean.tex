...
mathematics is entirely classical (Newton, Maclaurin, Sylvester); the contribution is the
...
\label{thm:newton}
For a real-rooted real polynomial $p$ of degree $n$ and every $1\le k\le n-1$,
...
Newton's inequalities are now a theorem in the library. Standing on them, five questions worth the
next effort.

\begin{enumerate}
\item \emph{Maclaurin, mechanically.} Maclaurin's mean chain follows from Newton by a short
argument. What is the smallest Lean bridge that turns the inequality proved here into the full chain
$p_1\ge p_2^{1/2}\ge\cdots\ge p_n^{1/n}$?
\item \emph{The sharp case.} Equality at an interior $k$ forces all roots equal. Formalizing that
needs the strict version of Rolle and of the discriminant; what is the cleanest route, and does the
weak-hypothesis base case still help or now get in the way?
\item \emph{Matching numbers as a corollary.} Composing \lean{newton\_inequality\_coeff} with the
Heilmann--Lieb real-rootedness of the matching polynomial~\cite{PartII} should print log-concavity
of the matching numbers in a few lines. How few?
\item \emph{The closure made explicit.} Differentiation and reversal both preserve real-rootedness.
What is the largest natural family of coefficient-window operations that do, and is ``enough to
reach any quadratic window'' a theorem one could state and check?
\item \emph{Beyond real roots.} The Hodge-theoretic log-concavity results~\cite{AHK2018} hold where
no real-rooted polynomial exists. Newton is the real-rooted floor; what is the smallest formalized
fragment of the matroid story that would sit one storey above it?
\end{enumerate}
