...
en cualquier asistente de pruebas: el dilogaritmo real con la identidad de reflexi\'on
...
efectiva de Lee--Yang $c_{\mathrm{ef}}=2/5$, la identidad dilogar\'itmica m\'as simple
del ansatz de Bethe termodin\'amico; la funci\'on de Clausen $\Cl$ y la \emph{constante
...
preguntas, en el orden en que realmente surgieron; cada secci\'on abre con su pregunta y
termina entregando al lector la siguiente. La Figura~\ref{fig:map} es el mapa. Si solo
va a leer una cosa, lea el Teorema~\ref{thm:clock} y la Figura~\ref{fig:clock}; el
inventario Lean es la Tabla~\ref{tab:inventory}, y reproducirlo todo son tres \'ordenes
...
\node[obj, right=14mm of hole] (li2) {$\Li$ y sus ecuaciones\\funcionales};
\node[obj, right=14mm of li2] (gold) {la escalera\\\'aurea};
\node[obj, below=9mm of gold] (circle) {el c\'irculo:\\par\'abola $+$ $\Cl$};
\node[obj, left=14mm of circle] (pos) {Fej\'er--Jackson:\\$\Cl>0$};
\node[obj, left=14mm of pos, fill=ggamber!25, draw=ggamber] (clock)
  {\textbf{el reloj que}\\\textbf{nunca suena}};
\draw[arr] (hole) -- node[q, above=0.5mm] {P1: ¿qu\'e sabe\\la serie?} (li2);
\draw[arr] (li2) -- node[q, above=0.5mm] {P2: ¿escalera\\sin Abel?} (gold);
\draw[arr] (gold) -- node[q, right=1mm] {P3: ¿qu\'e pasa\\en el c\'irculo?} (circle);
\draw[arr] (circle) -- node[q, above=0.5mm] {P4: ¿por qu\'e es positiva\\la mitad salvaje?} (pos);
\draw[arr] (pos) -- node[q, above=0.5mm] {P5: ¿qu\'e cosa f\'isica\\late con esto?} (clock);
\end{tikzpicture}
\caption{El viaje, pregunta a pregunta. Cada nodo es un bloque de Lean sin
\lean{sorry}; cada flecha es la pregunta que produjo el bloque siguiente.}
\label{fig:map}
\end{figure}
...
Mathlib --- la biblioteca matem\'atica del asistente de pruebas Lean~4, sobre la que se
...
\emph{todos} los asistentes de pruebas (Lean, Rocq/Coq, Isabelle); las bibliotecas
...
\emph{Cuenta hist\'orica.} La funci\'on es de Euler (1768); la identidad de reflexi\'on
...
\emph{Cuenta hist\'orica.} Los valores son de Landen (1780); la suma de dos t\'erminos
es de la memoria de Rogers de 1907~\cite{rogers}. Su papel como carga central pertenece
al ansatz de Bethe termodin\'amico de los a\~nos
...
termodin\'amico de Lee--Yang, $c_{\mathrm{ef}}=\Lrog(1/\varphi^2)/\Lrog(1)=2/5$ ---
...
\emph{Cuenta hist\'orica.} Clausen introdujo su funci\'on en 1832~\cite{clausen}; la
...
\;=\;0{,}9159\ldots,
\]
la \textbf{constante de Catalan}, ausente de todos los asistentes de pruebas hasta ahora
...
\emph{Cuenta hist\'orica.} Fej\'er plante\'o y prob\'o parcialmente la positividad de
...
$\sin\theta+\tfrac12\sin2\theta=\sin\theta\,(1+\cos\theta)>0$ en $(0,\pi)$. La gracia de
la formalizaci\'on es que esa misma certeza cubre ahora todo $M$.
...
\emph{El giro --- el grande.} Demos un paso atr\'as y miremos lo que tenemos en las
manos. Una suma $\sum p_n e^{-in\theta}$ con pesos positivos $p_n\propto 1/n^2$, cuya
parte imaginaria acabamos de probar que nunca se anula en $(0,\pi)$. Un f\'isico lee esa
expresi\'on al instante: es la \emph{autocorrelaci\'on de un estado cu\'antico} --- el
producto interno entre un estado y su propia evoluci\'on temporal, para poblaciones
$p_n$ sobre niveles de energ\'ia equiespaciados. El c\'irculo llevaba todo el tiempo
midiendo \emph{tiempo}, y nuestro teorema de positividad es una afirmaci\'on sobre un
sistema cu\'antico que no puede alcanzar un estado ortogonal. ¿C\'omo de r\'apido
\emph{puede} alcanzarlo cualquier sistema cu\'antico?

\section{P5: ¿qu\'e cosa f\'isica late con esta funci\'on?}\label{sec:clock}

\emph{Cuenta hist\'orica.} Mandelstam y Tamm acotaron el tiempo de ortogonalizaci\'on
...
formalizamos --- el espacio de Hilbert es contabilidad. La ortogonalidad entra como dos
...
Ambas cotas del Teorema~\ref{thm:qsl} se satisfacen vac\'iamente: sobre este estado, los
...
c\'irculo: el estado nunca llega ni a \emph{mitad} de camino de la distinguibilidad
(Figura~\ref{fig:clock}). Un reloj con presupuesto energ\'etico infinito cuya aguja
nunca completa una campanada --- porque el presupuesto se gasta en una cola espectral
...
junio de 2026, la primera formalizaci\'on en cualquier asistente de pruebas de cada
...
polilogaritmos o en QSL; y (iv)~la observaci\'on de la Observaci\'on~\ref{rem:ek}.
...
\begin{tabular}{@{}llr@{}}
\toprule
Archivo & Contenido & L\'ineas\\
\midrule
\lean{Dilog/Basic.lean} & $\Li$: serie, derivada, reflexi\'on, Landen,&\\
& duplicaci\'on, $\Li(\frac12)$, escalera \'aurea, Rogers $\Lrog$, Lee--Yang & 581\\
\lean{Dilog/Clausen.lean} & $\Cl$, $G$ de Catalan, par\'abola de Bernoulli,
$\Cl\ge\sin\theta/2$,&\\
& $\Cl(2\pi-\theta)=-\Cl(\theta)$, Teorema~\ref{thm:clock} & 332\\
\lean{Dilog/FejerJackson.lean} & desigualdad de Fej\'er--Jackson & 189\\
\lean{QSL/Basic.lean} & Margolus--Levitin (+ la desigualdad del coseno) & 238\\
\lean{QSL/MandelstamTamm.lean} & Mandelstam--Tamm, forma $L^1$ & 134\\
\midrule
&& 1\,474\\
\bottomrule
\end{tabular}
