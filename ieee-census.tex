...
length $n=648$, and, as the first such diagnostic on a \emph{quantum} LDPC code, of the IBM
...
\emph{(2) The first such diagnostic on a deployed quantum code.} We run the census, unchanged,
...
\eqref{eq:lift} exactly for all $k=1,\dots,8$ on the gross/$[[72,12,6]]$ pair; at $k=6$,
$1728=864+864$. Every base $6$-cycle has signing voltage $+1$ and lifts to two cover
$6$-cycles, so $c_6$ doubles. Consequently the certified short-cycle profile of a BB \emph{code
family} is determined by its base together with the covering signing, through the Artin--Ihara
$L$-function factorization---a practical handle on whole towers rather than one code at a time.
...
the deployed IEEE~802.11n codes and, for the first time, to a deployed quantum LDPC code, with a
