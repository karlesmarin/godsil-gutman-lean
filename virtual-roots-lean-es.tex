...
\begin{tikzpicture}[
  ...
  \node[obj] (def) {¿qu\'e es una\\ra\'iz virtual?};
  \node[obj,right=33mm of def] (build) {constr\'uyela:\\recursi\'on $\Rd_d$};
  \node[obj,right=33mm of build] (resist) {¿qu\'e se\\resiste?};
  \node[obj,below=15mm of resist] (give) {entrelazado\\de Rolle};
  \node[obj,left=33mm of give] (next) {recuento\\exacto (luego)};
  \node[obj,left=33mm of next,fill=ggamber!25,draw=ggamber] (done)
    {\textbf{$d$ ra\'ices,}\\\textbf{ordenadas,}\\\textbf{verificadas}};
  \draw[arr] (def) -- node[q,above=2pt] {P1:\\¿exact.\,$d$?} (build);
  \draw[arr] (build) -- node[q,above=2pt] {P2:\\¿c\'omo?} (resist);
  \draw[arr] (resist) -- node[q,right=2pt,fill=none] {P3:\\¿resiste?} (give);
  \draw[arr] (give) -- node[q,above=2pt] {P4:\\¿luego?} (next);
  \draw[arr] (next) -- (done);
\end{tikzpicture}
\caption{El mapa del lector. De la definici\'on de ra\'iz virtual a una familia certificada y ordenada
de exactamente $d$ con el entrelazado de Rolle; el recuento exacto de Budan--Fourier es la cuenta que
se entrega a la secuela. Cada nodo sombreado es un bloque de Lean \lean{sorry}-free.}
\label{fig:readermap}
\end{figure}
