...
eslab\'on tiene su secci\'on, una figura o una tabla trabajada, y un ejemplo lo bastante
peque\~no para comprobarlo a mano. Los l\'imites honestos est\'an en la Secci\'on~\ref{sec:boundary}.
...
\begin{tikzpicture}[
  ...
  \node[hist]  (a) at (0,1.5)    {1812: el determinante\\de un producto\ \S\ref{sec:cb}};
  \node[hist]  (b) at (4.05,1.5) {1847: un circuito\\pide \'arboles\ \S\ref{sec:intro}};
  \node[theme] (c) at (8.1,1.5)  {la afirmaci\'on\\$\det L_0=\tau(G)$\ \S\ref{sec:result}};
  \node[theme] (d) at (12.15,1.5){el laplaciano\\factoriza: $NN^{\mathsf T}$\ \S\ref{sec:gram}};
  \node[theme] (e) at (12.15,-0.4){el censo\\$\sum_S(\det N_{0,S})^2$\ \S\ref{sec:sum}};
  \node[theme] (f) at (8.1,-0.4) {la dicotom\'ia:\\qui\'en sobrevive\ \S\ref{sec:dichotomy}};
  \node[theme] (g) at (4.05,-0.4){comprobado:\\Sage + axiomas\ \S\ref{sec:verification}};
  \node[last]  (h) at (0,-0.4)   {l\'imites, usos,\\preguntas\ \S\ref{sec:boundary}--\ref{sec:questions}};
  \draw[flow] (a) -- (b);
  \draw[flow] (b) -- (c);
  \draw[flow] (c) -- (d);
  \draw[flow] (d) -- (e);
  ...
  \draw[flow] (g) -- (h);
\end{tikzpicture}
\caption{El mapa del lector: los temas del art\'iculo en su orden l\'ogico---dos ingredientes
cl\'asicos, la afirmaci\'on, la factorizaci\'on que la descompone, el censo, la decisi\'on, las
comprobaciones, y lo que queda.}
\label{fig:readermap}
\end{figure}
...
resistencias~\cite{Kirchhoff1847}---y encontr\'o que el denominador com\'un de su soluci\'on,
el determinante que la regla de Cramer coloca bajo cada corriente, cuenta algo puramente
combinatorio: los \'arboles generadores del grafo de la red. En lenguaje
...
El teorema llev\'o despu\'es una doble vida. Borchardt fue el primero, en 1860, en obtener la
cuenta $n^{n-2}$ de los \'arboles etiquetados evaluando un determinante~\cite{Borchardt1860};
la nota breve de Cayley de 1889, que dio nombre a la f\'ormula y acredita a Borchardt, exhibe
la cuenta que el determinante matricial de los \'arboles produce sobre el grafo
...
cinco ficheros de la Tabla~\ref{tab:loadbearing}, con $35$ declaraciones en total ($28$
teoremas y lemas, seis definiciones, una instancia local), que importan once ficheros de
Mathlib (el
...
\emph{Dispersi\'on espectral}: muestrear aristas por resistencia efectiva preserva
aproximadamente la forma cuadr\'atica del laplaciano, con alta
probabilidad~\cite{SpielmanSrivastava2011}; la cantidad por la que se muestrea es, otra vez,
...
cuadrado vale $0$ o $\pm1$)). La dicotom\'ia lo establece para los menores \emph{maximales} de
la matriz reducida $N_0$; el hecho cl\'asico es que las matrices de incidencia orientadas son
totalmente unimodulares, en todos los \'ordenes. ?`Conectar\'ia demostrar y registrar la
unimodularidad total completa este desarrollo con la parte de la biblioteca de sabor
optimizaci\'on, y se extiende el argumento de la arista padre por debajo del orden m\'aximo?
\end{itemize}
...
\textbf{9}, cahier~16 (1813). Le\'ida el 30 de noviembre de 1812.
