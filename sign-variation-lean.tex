...
\matrix (m) [matrix of nodes, row sep=2.2mm, column sep=7mm, nodes={anchor=west, inner sep=2pt}]{
   & \textit{sequence built from } $p=x^3-x$ & \textit{the engine $\V$ gives} & \textit{drop counts} \\
...
\begin{scope}[on background layer]
  \node[draw=ggteal,thick,rounded corners=3pt,fill=ggband!35,fit=(m-2-3)(m-4-3),inner sep=4pt,
    label={[font=\scriptsize\itshape,text=ggteal]above:one engine $\V=\lean{signChanges}$}] {};
\end{scope}
\end{tikzpicture}
