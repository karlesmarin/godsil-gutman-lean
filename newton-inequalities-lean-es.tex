...
Sylvester); la contribuci\'on es la formalizaci\'on---junto con cuatro bloques reutilizables
...
\label{thm:newton}
Para un polinomio real con ra\'ices reales $p$ de grado $n$ y todo $1\le k\le n-1$,
...
Las desigualdades de Newton son ya un teorema en la biblioteca. Sobre ellas, cinco preguntas que
valen el siguiente esfuerzo.

\begin{enumerate}
\item \emph{Maclaurin, mec\'anicamente.} La cadena de medias de Maclaurin se sigue de Newton por un
argumento corto. ¿Cu\'al es el puente Lean m\'as peque\~no que convierte la desigualdad probada aqu\'i
en la cadena completa $p_1\ge p_2^{1/2}\ge\cdots\ge p_n^{1/n}$?
\item \emph{El caso afilado.} La igualdad en un $k$ interior fuerza que todas las ra\'ices sean
iguales. Formalizarlo necesita la versi\'on estricta de Rolle y del discriminante; ¿cu\'al es la ruta
m\'as limpia, y el caso base de hip\'otesis d\'ebil sigue ayudando o ahora estorba?
\item \emph{N\'umeros de emparejamientos como corolario.} Componer \lean{newton\_inequality\_coeff}
con la real-radicalidad de Heilmann--Lieb del polinomio de emparejamientos~\cite{PartII} deber\'ia
imprimir la log-concavidad de los n\'umeros de emparejamientos en pocas l\'ineas. ¿Cu\'antas?
\item \emph{La clausura, explicitada.} La derivaci\'on y la reversi\'on preservan ambas la
real-radicalidad. ¿Cu\'al es la mayor familia natural de operaciones sobre ventanas de coeficientes
que lo hacen, y es ``suficiente para alcanzar cualquier ventana cu\'adratica'' un teorema que se
pueda enunciar y verificar?
\item \emph{M\'as all\'a de las ra\'ices reales.} Los resultados de log-concavidad por teor\'ia de
Hodge~\cite{AHK2018} valen donde no existe polinomio con ra\'ices reales. Newton es el suelo con
ra\'ices reales; ¿cu\'al es el fragmento formalizado m\'as peque\~no de la historia de matroides que
se asentar\'ia un piso por encima de \'el?
\end{enumerate}
