...
\emph{A graph's matching polynomial is the characteristic polynomial of no matrix---so where do the
power sums of its roots live, and why are they a count of walks?} The prequel answered the first
clause with a tree and left the second as a promissory note; this paper redeems it. The route has a
long history---four centuries---with one twist worth flagging in advance: its final step does without
the oldest tool in the story.
...
roots real anyway~\cite{HeilmannLieb}, even though it is the characteristic polynomial of no symmetric
matrix unless the graph is a forest. In 1981 Godsil resolved the paradox with a single device, the
...
second~\cite{PartIII}, and this paper carries the rest. The closure, it turns out, uses no Newton
recursion (Section~\ref{sec:boundary}): the first bead---Girard's---is the one the proof can do
without.

\paragraph{The polynomial with no matrix.} A finite graph $G$ on $n$ vertices carries a matching
polynomial $\mu(G,x)=\sum_k(-1)^k m_k(G)x^{n-2k}$, with $m_k(G)$ the number of $k$-edge
matchings~\cite{Farrell1979,LovaszPlummer}. Heilmann and Lieb proved its roots
$\theta_1,\dots,\theta_n$ real~\cite{HeilmannLieb}; but $\mu(G)$ is the characteristic polynomial of
a fixed symmetric matrix only when $G$ is a forest. Their power sums $p_k(G)=\sum_i\theta_i^k$ are
therefore the moments of a \emph{ghost} spectrum---real numbers behaving like eigenvalues of a matrix
that is not there. Godsil's moment theorem says where they come from: they \emph{count walks}.

\paragraph{One tree, and a promissory note.} Godsil's device~\cite{Godsil1981a,Godsil1981b,GodsilBook}
...
careful to mark its boundary: the equality of this count with the ghost moments $p_k$---the
\emph{spectral} half---was mapped in its closing section, not formalized. This paper builds it.
...
\paragraph{What is new, and what is not.} The mathematics is Godsil's, four decades old. The
contribution is the machine-checked completion of the moment theorem---the first I am aware of in any
proof assistant---and, along the way, a handful of general, Mathlib-absent lemmas kept in canonical
graph-free form: a principal-minor determinant collapse for an arbitrary finite index, a
reflection-of-a-sum identity, and the reverse of a product of linear factors. The headline,
...
The entire trace side is now one reflected matching polynomial
(\lean{mk\_treeLikeWalkCount\_mul\_reverse\_eq}). Using $\reflect_n$, rather than $\rev$, for the
numerator is what lets us avoid ever computing the degree of $X\mu'$.
...
against exact arithmetic, and the same checks fix the theorem's two sides side by side.
...
the proofs and this prose. Every theorem named here compiles \lean{sorry}-free against the pinned
Mathlib revision~\cite{Mathlib} and depends only on the three standard axioms, re-verifiable with
\lean{\#print axioms}.

\paragraph{A false green, and the lesson banked.} One step in this work was, for a time, silently
broken. The reversed-divisibility identity (\lean{godsil\_resolvent\_charpoly\_form}) had been
committed with a build that replayed a stale compiled object: \lean{lake build} reported success while
the source no longer compiled, because no later edit had forced its module to rebuild. A clean
recompilation exposed two genuine defects---an instance-diamond on the path-tree matching polynomials,
and a stray explicit argument---both since fixed. The discipline this enforces is simple and worth
stating: a per-target build passing is not a proof that a theorem compiles; only a clean elaboration
of the file itself is. Every identity in this paper was re-checked by elaborating its module from
source, not by trusting an incremental build.

\paragraph{Where it applies, and how.} The moment theorem is the bridge between two ways of reading a
graph: as a combinatorial object whose walks one counts, and as the carrier of a ghost spectrum whose
moments one bounds. It is precisely the identity invoked, usually tacitly, wherever one of these is
...
And one to keep. \emph{When a machine certifies that a graph's tree-like walks are exactly the power
sums of a spectrum no matrix carries, has it proved that the ghost is real---or only named the precise
sense in which a forest dreams the graph a matrix it never had?}
