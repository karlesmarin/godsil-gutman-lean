...
vertex in disguise. The bridge is a single invariant: as one walks $T(G,v)$, running \lean{liftSeq} on
the projected walk tracks the current path-tree vertex.
...
the pinned Mathlib revision~\cite{Mathlib} and depends only on the three standard axioms, a fact the
reader can re-verify with \lean{\#print axioms}. The corrected ``tree-like'' definition is itself a small
