...
\begin{tikzpicture}[
  ...
  \node[obj] (def) {what is a\\virtual root?};
  \node[obj,right=33mm of def] (build) {build it:\\$\Rd_d$ recursion};
  \node[obj,right=33mm of build] (resist) {what resists\\formalizing?};
  \node[obj,below=15mm of resist] (give) {Rolle\\interlacing};
  \node[obj,left=33mm of give] (next) {the exact\\count (next)};
  \node[obj,left=33mm of next,fill=ggamber!25,draw=ggamber] (done)
    {\textbf{$d$ roots,}\\\textbf{sorted,}\\\textbf{checked}};
  \draw[arr] (def) -- node[q,above=2pt] {Q1:\\exactly $d$?} (build);
  \draw[arr] (build) -- node[q,above=2pt] {Q2:\\how?} (resist);
  \draw[arr] (resist) -- node[q,right=2pt,fill=none] {Q3:\\resists?} (give);
  \draw[arr] (give) -- node[q,above=2pt] {Q4:\\then?} (next);
  \draw[arr] (next) -- (done);
\end{tikzpicture}
\caption{The reader's map. From the definition of a virtual root to a certified, sorted family of
exactly $d$ of them with the Rolle interlacing; the exact Budan--Fourier count is the bead handed to
the sequel. Every shaded node is a block of \lean{sorry}-free Lean.}
\label{fig:readermap}
\end{figure}
