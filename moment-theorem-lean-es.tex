...
cancelaci\'on de serie geom\'etrica contra el producto de factores invertidos, al \emph{mismo} objeto
$\overline{X\mu'}$. Ambos coinciden; cancelar la unidad $\overline{\mu}$ y leer el coeficiente de
$X^k$ es el teorema. Registro una sorpresa honesta: la conclusi\'on no necesita \emph{ninguna}
identidad de Newton univariada---la ruta que la entrega anterior predijo---sino solo una contabilidad
de reflexi\'on a grado fijo de polinomios invertidos. El resultado depende solo de los tres axiomas
...
una identidad por v\'ertice que vuelve el lado de la traza polinomios de emparejamientos invertidos
(Secci\'on~\ref{sec:trace}), una cancelaci\'on de serie geom\'etrica que vuelve el lado de los
emparejamientos lo mismo (Secci\'on~\ref{sec:match}), y una extracci\'on de coeficiente una vez
cancelada una unidad (Secci\'on~\ref{sec:weld}). La sorpresa honesta---sin Newton---es la
...
\emph{El polinomio de emparejamientos de un grafo no es el polinomio caracter\'istico de ninguna
matriz---\,?`d\'onde viven entonces las sumas de potencias de sus ra\'ices, y por qu\'e son un conteo
de caminos?} La entrega anterior respondi\'o la primera cl\'ausula con un \'arbol y dej\'o la segunda
como pagar\'e; este art\'iculo lo salda. La ruta tiene una historia larga---cuatro siglos---con un
giro que conviene anunciar de antemano: su paso final prescinde de la herramienta m\'as antigua de la
historia.
...
modos~\cite{HeilmannLieb}, aunque no es el polinomio caracter\'istico de ninguna matriz sim\'etrica
salvo que el grafo sea un bosque. En 1981 Godsil resolvi\'o la paradoja con un solo dispositivo, el
...
segunda~\cite{PartIII}, y este art\'iculo lleva el resto. El cierre, resulta, no usa recursi\'on de
Newton (Secci\'on~\ref{sec:boundary}): la primera cuenta---la de Girard---es la que la demostraci\'on
puede ahorrarse.

\paragraph{El polinomio sin matriz.} Un grafo finito $G$ de $n$ v\'ertices porta un polinomio de
emparejamientos $\mu(G,x)=\sum_k(-1)^k m_k(G)x^{n-2k}$, con $m_k(G)$ el n\'umero de emparejamientos de
$k$ aristas~\cite{Farrell1979,LovaszPlummer}. Heilmann y Lieb demostraron que sus ra\'ices
$\theta_1,\dots,\theta_n$ son reales~\cite{HeilmannLieb}; pero $\mu(G)$ es el polinomio
caracter\'istico de una matriz sim\'etrica fija solo cuando $G$ es un bosque. Sus sumas de potencias
$p_k(G)=\sum_i\theta_i^k$ son, por tanto, los momentos de un espectro \emph{fantasma}: n\'umeros
reales que se comportan como autovalores de una matriz que no est\'a. El teorema del momento de Godsil
dice de d\'onde vienen: \emph{cuentan caminos}.

\paragraph{Un \'arbol, y un pagar\'e.} El dispositivo de Godsil~\cite{Godsil1981a,Godsil1981b,GodsilBook}
...
fantasma $p_k$---la mitad \emph{espectral}---qued\'o mapeada en su secci\'on de cierre, no formalizada.
Este art\'iculo la construye.
...
\paragraph{Qu\'e es nuevo, y qu\'e no.} La matem\'atica es de Godsil, de hace cuatro d\'ecadas. La
contribuci\'on es la conclusi\'on verificada por m\'aquina del teorema del momento---la primera que
conozco en cualquier asistente de demostraci\'on---y, de camino, un pu\~nado de lemas generales
ausentes de Mathlib en forma can\'onica y libre de grafos: un colapso de determinante a un menor
principal para un \'indice finito arbitrario, una identidad de reflexi\'on de una suma, y el invertido
de un producto de factores lineales. El resultado, \lean{matchingPowerSum\_eq\_treeLikeWalkCount},
...
$\mu(T)=\det(xI-A(T))$ vuelve cada polinomio de emparejamientos invertido un $\charpolyRev$; cancelando
...
contabilidad de reflexi\'on rinde: todo $\mu(G-v)$ tiene el mismo grado $n$, as\'i que las inversiones
...
fijan los dos lados del teorema lado a lado.
...
$\sum_\theta(1-\theta X)^{-1}$ multiplicada por $\prod_\theta(1-\theta X)$ se telescopa, por el valor
de la cancelaci\'on de un factor, en la suma ``dejando uno fuera''
...
El autor fij\'o los objetivos y adjudic\'o la matem\'atica; el asistente (Claude, Anthropic) redact\'o
las demostraciones y esta prosa. Todo teorema nombrado aqu\'i compila \lean{sorry}-libre contra la
revisi\'on fijada de Mathlib~\cite{Mathlib} y depende solo de los tres axiomas est\'andar,
re-verificables con \lean{\#print axioms}.

\paragraph{Un verde falso, y la lecci\'on guardada.} Un paso de este trabajo estuvo, por un tiempo,
silenciosamente roto. La identidad de divisibilidad invertida (\lean{godsil\_resolvent\_charpoly\_form})
se hab\'ia confirmado con una compilaci\'on que reprodujo un objeto compilado obsoleto: \lean{lake
build} report\'o \'exito mientras la fuente ya no compilaba, porque ninguna edici\'on posterior hab\'ia
forzado a su m\'odulo a recompilar. Una recompilaci\'on limpia expuso dos defectos reales---un diamante
de instancias en los polinomios de emparejamientos del \'arbol de caminos, y un argumento expl\'icito
de m\'as---ambos ya corregidos. La disciplina que esto impone es simple y vale enunciarla: que una
compilaci\'on por objetivo pase no es prueba de que un teorema compile; solo lo es una elaboraci\'on
limpia del propio fichero. Cada identidad de este art\'iculo se re-verific\'o elaborando su m\'odulo
desde la fuente, no confiando en una compilaci\'on incremental.

\paragraph{D\'onde se aplica, y c\'omo.} El teorema del momento es el puente entre dos maneras de leer
un grafo: como objeto combinatorio cuyos caminos se cuentan, y como portador de un espectro fantasma
cuyos momentos se acotan. Es precisamente la identidad invocada, casi siempre t\'acitamente, all\'i
...
cumple $\theta_{\max}=\lim_k p_{2k}^{1/2k}$, y el teorema del momento vuelve cada $p_{2k}$ un
\emph{conteo de caminos} en el \'arbol de caminos---una cantidad combinatoria expl\'icitamente acotada,
...
El mismo bosque hace ahora un tercer trabajo: vuelve el conteo \emph{igual} al espectro. Un cimiento, y
el cierre de un arco de cuatro partes.
...
Y una para guardar. \emph{Cuando una m\'aquina certifica que los caminos tipo \'arbol de un grafo son
exactamente las sumas de potencias de un espectro que ninguna matriz porta, \,?`ha demostrado que el
fantasma es real---o solo ha nombrado el sentido preciso en que un bosque sue\~na al grafo una matriz
que nunca tuvo?}
