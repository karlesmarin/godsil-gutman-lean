...
\medskip
\noindent El desarrollo es un \'unico fichero Lean, \lean{VirtualRoots.lean}; sus lemas que soportan el
peso est\'an reunidos en la Secci\'on~\ref{sec:blocks}, y sus l\'imites---sobre todo el recuento exacto
diferido---se enuncian en la Secci\'on~\ref{sec:boundary}. Si solo lees dos cosas, lee el entrelazado
(Teorema~\ref{thm:interlace}) y la Figura~\ref{fig:fillin}.
...
El objeto es de Gonz\'alez-Vega, Lombardi y Mah\'e~\cite{glm}, y es el portador natural del recuento de
Budan--Fourier, vuelto igualdad a trav\'es de \'el por Coste, Lajous, Lombardi y Roy~\cite{coste}. Este
paper formaliza la parte estructural de esa historia---la construcci\'on, el recuento, el orden y el
entrelazado de Rolle---y es honesto, en la Secci\'on~\ref{sec:boundary}, sobre el recuento aritm\'etico
que deja a la secuela. Llegu\'e a \'el como llegu\'e a Sturm~\cite{sturmlean} y a
Budan--Fourier~\cite{bflean}: preguntando qu\'e resultados cl\'asicos de recuento de ra\'ices le faltan
a una biblioteca madura, y hallando las ra\'ices virtuales ausentes de todo asistente que pude buscar.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lc@{}}
    ...
    \midrule
    \rowcolor{ggband}
    L\'ineas en \lean{VirtualRoots.lean} & $369$ \\
    Teoremas / definiciones p\'ublicas ($+$ privado) & $16\,/\,2$ ($+1$) \\
    \rowcolor{ggband}
    Dependencias de Mathlib (imports) & $5$ m\'odulos \\
    ...
    \rowcolor{ggband}
    Tiempo de compilaci\'on (Mathlib precompilado) & ${\approx}\,8$\,s \\
    Toolchain & Lean~4 (godsil pin v4.30) sobre Mathlib \\
    \bottomrule
  \end{tabular}
  \arrayrulecolor{black}
  \caption{El desarrollo, medido. Build: \lean{lake env lean VirtualRoots.lean}, exit~0; el tiempo de
  compilaci\'on es para verificar el fichero contra un Mathlib precompilado. La huella es
  deliberadamente peque\~na---cinco m\'odulos de an\'alisis/topolog\'ia y nada de \'algebra
  pesada---porque la construcci\'on es elemental.}
  \label{tab:metrics}
\end{table}
...
Tres hechos que este desarrollo no resuelve marcan hacia d\'onde apunta; cada uno es un lugar donde el
trabajo se queda corto, y cada uno es el comienzo del siguiente.
