...
among them. (2)~It states their common local-drop law,
...
Lean engine, the two first-in-Lean formalizations it carries, and this exact axiomatic placement of the
tower. Every headline theorem depends only on \lean{propext}, \lean{Classical.choice}, \lean{Quot.sound}.
\end{abstract}
...
\emph{Sturm} (\cite{sturmlean}; Figure~\ref{fig:sturm}). The signed-remainder chain
...
\emph{Budan--Fourier} (\cite{budanlean}; Figure~\ref{fig:tower}). The derivative tower
...
Sturm; its distinctive face is the double root and the phantom even drop (Figure~\ref{fig:tower}).
...
For $x^3-x$ the coefficients $1,0,-1,0$ give one sign change and one positive root, $x=1$.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{sturm_fig1_staircase}
\caption{The Sturm instance (from~\cite{sturmlean}): for $p=x^3-x$ the staircase $\V$ drops by exactly
one at each root and does not move at a zero of an interior chain member (the $\pm1/\sqrt3$ guides),
where the antipodal neighbours absorb it. Distinct-root count, exact.}
\label{fig:sturm}
\end{figure}
...
\caption{The Budan--Fourier instance (from~\cite{budanlean}): \emph{(A)} the tower drop counts
multiplicity (step of two at a double root); \emph{(B)} at a critical point that is not a root the drop
is still even---the trace of a conjugate pair. Bound with multiplicity, even parity.}
\label{fig:tower}
\end{figure}
...
\begin{tikzpicture}[
  ...
  % Sturm side
  \node[font=\bfseries\small,text=ggteal] at (1.4,2.2) {Sturm chain: one sandwiched zero};
  ...
  % BF side
  \node[font=\bfseries\small,text=ggplum] at (6.1,2.2) {Tower: a vanishing block};
...
running through it; and Proposition~\ref{prop:notsturm}, the precise placement of the derivative tower
...
Machine-Checked in Lean~4}, 2026 (companion paper).
\end{thebibliography}
