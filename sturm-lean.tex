...
based on searches of Loogle and Mathlib in June~2026 (Section~\ref{sec:boundary})---this is the
first proof of Sturm's theorem in Lean. The contribution is therefore the
...
first-in-Lean and not a first-in-any-system;\footnote{Priority claim based on searches of Loogle
and a \lean{grep} of Mathlib as of June~2026; the prior formalizations in Coq, Isabelle/HOL and
HOL~Light are credited in Section~\ref{sec:boundary}.} the value is in adding a foundational tool to
a library that lacked it, and in the reusable pieces the proof leaves behind.
...
\lean{FlankReduce}~$L\;L'$ holds when $L'$ is obtained from $L$ by deleting entries each caught
between two nonzero opposite signs: one step takes
$\textit{pre}\mathbin{+\!\!+}[a,m,b]\mathbin{+\!\!+}\textit{rest}$ to
$\textit{pre}\mathbin{+\!\!+}[a,b]\mathbin{+\!\!+}\textit{rest}$ whenever $a,b\ne0$ and $a\ne b$, and
the relation is the reflexive closure of iterating it. Two facts then divide the labour cleanly. \emph{Combinatorial:} flank-reduction preserves the
...
\emph{weighted} by the sign of a second polynomial $q$ on them---the ``Tarski query''
$\sum_{p(x)=0}\operatorname{sign} q(x)$---which equals the Cauchy index of $p'q/p$ (the basic case
$q=1$, the Cauchy index of $p'/p$, being Sturm's own count). This is the engine of root counting
\emph{with multiplicity}, of the non-squarefree case, and ultimately of Tarski's quantifier elimination for
...
\emph{What Mathlib already provided}, making several steps almost free: the Euclidean-domain
...
substrate, in other words, was already there.

\emph{What had to be built from scratch.} Absent everywhere in Lean were the sign-variation theory
and the whole local-to-global crossing argument. The one genuinely fiddly step---independent of the
system---was the list-recursion coupling that \lean{FlankReduce} resolves (Section~\ref{sec:q3}),
not the analysis.

\emph{On comparing systems.} I did not port Cohen's or Eberl's proofs, so I make no claim about
relative proof length or difficulty: a fair comparison would mean re-doing the work in each system,
which I have not done. What I can say is that the approaches differ in setting---Cohen's is embedded
in exact real arithmetic, Eberl's is executable, Li--Paulson's targets the Cauchy index---and that,
in Lean, the cost lived in the combinatorics of the count, not in the polynomial analysis Mathlib
already supports.
...
\end{verbatim}
Lean \lean{v4.30.0-rc2}, Mathlib pin \lean{701fb6e9}. (The repository
\texttt{godsil-gutman-lean} is the umbrella for this series of Lean formalizations; the present work
is the single file \lean{Sturm.lean} within it.) The theorem is \lean{Sturm.sturm} in
