...
$\pm1$. The proof is a single chain (Figure~\ref{fig:road}); every link has its own section, its
own figure, and a worked example small enough to check by hand. The honest limits are in
...
network~\cite{Kirchhoff1847}---and found that the quantity controlling their solvability counts
something purely combinatorial: the spanning trees of the network's graph. In modern dress, with
...
The theorem then led a double life. Borchardt gave the first determinantal proof in
1860~\cite{Borchardt1860}; Cayley's celebrated $n^{n-2}$ count of labelled trees, in a short
note of 1889, is the formula evaluated on the complete graph~\cite{Cayley1889}. In 1940,
...
files of Table~\ref{tab:loadbearing}, containing $35$ declarations in total (theorems, lemmas,
two definitions, and two scoped instances), importing eleven Mathlib files (the
...
\emph{Spectral sparsification}: sampling edges by effective resistance preserves the Laplacian
spectrum~\cite{SpielmanSrivastava2011}; the quantity being sampled by is, again, a ratio of the
...
is $0$ or $\pm1$''. The dichotomy proves a sharp instance of it for $N$. Would registering
$N$'s total unimodularity connect this development to the optimization-flavoured part of the
library, and does the parent-edge argument give the shortest such proof?
\end{itemize}
...
\textbf{9}, cahier~16 (1813), 280--354. Read 30 November 1812.
