...
\begin{remark}[the tower is not a Sturm chain]\label{rem:notsturm}
It is worth pinning down how far the tower sits from the classical setting. Eisermann's abstract
\emph{Sturm chain}~\cite{eisermann} must satisfy one axiom (his Def.~3.10): whenever an \emph{interior}
member $S_k$ vanishes at a point, its neighbours are strictly opposite there, $S_{k-1}\,S_{k+1}<0$---so
no two consecutive members vanish together. The derivative tower fails this at every multiple root. For
$p_A=(x-1)^2(x+1)$ the interior member $p_A'$ vanishes at $x=1$, yet $p_A(1)\,p_A''(1)=0\not<0$, because
$p_A$ and $p_A'$ vanish there \emph{at once}. So the tower is not a Sturm chain; the single-pair
head-drop that proves Sturm's theorem does not apply; and the block collapse/alternation of
Section~\ref{sec:q2}---\lean{Rseq}/\lean{Lseq}---is exactly the local law that takes over once
Eisermann's antipodal condition is allowed to degenerate. This is the precise sense in which the two
papers' local analyses differ, and the starting point of the open question in
...
Sturm's counts are facets of one sign-variation principle is classical, not mine---said plainly by
Akritas~\cite{akritas} and Basu--Pollack--Roy~\cite{bpr}---and the deeper invariant, the \emph{Cauchy
index}, is axiomatised through abstract \emph{Sturm chains} by Eisermann~\cite{eisermann} and has
\emph{already} been machine-checked, in Isabelle/HOL, by Li and Paulson~\cite{liCauchy}. What is missing
is a \emph{Lean} formalisation. And here the tower is not merely one more instance: Eisermann's
Sturm-chain axiom (his Def.~3.10) demands that whenever an \emph{interior} member vanishes, its
neighbours be strictly opposite, $S_{k-1}\,S_{k+1}<0$---so no two consecutive members may vanish
together. The derivative tower \emph{violates} this at every multiple root, where $p$ and $p'$ vanish
at once; it is not a Sturm chain in the classical sense. The honest open question is then sharp: what is
the weakest axiom that still forces the local drop, with the block collapse/alternation of
Section~\ref{sec:q2} (the lemmas \lean{Rseq\_spec}/\lean{Lseq\_spec}) as the general local law and
Eisermann's antipodal condition as its non-degenerate special case?
