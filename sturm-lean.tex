...
things, read Theorem~\ref{thm:sturm} and Figure~\ref{fig:staircase}; everything else is the
argument that connects them, and the argument is the point.
...
procedures for the first-order theory of the reals that grew out of Tarski's work. Three
centuries of effort to count roots end, in effect, with a subtraction.
...
zero otherwise---matches the number of $p$-roots in the peeled piece. The staircase is assembled
one step at a time.
...
\bottomrule
\end{tabular}
\caption{The load-bearing results, all \lean{sorry}-free in \lean{Sturm.lean} of the repository
(around sixty named lemmas on Mathlib). The axioms of every entry are \lean{propext},
...
\emph{Prior formalizations.} The priority claim is narrow and rests on a documented search, not a
feeling. Sturm's theorem has been formalized before: in Coq, within Cohen's real-algebraic-number
development~\cite{cohen}; in Isabelle/HOL by Eberl~\cite{eberl} and, in the Sturm--Tarski form, by
Li and Paulson~\cite{litarski,libudan}; and in HOL~Light. What I searched for (June~2026) and did
not find was any formalization of the Sturm sequence or Sturm's theorem in \emph{Lean}/Mathlib
(Loogle and \lean{grep}: \lean{Polynomial.signVariations} for Descartes' rule exists; no
\lean{sturm}, no signed-remainder sequence). The claim is therefore: first in Lean, to the best of
my knowledge, and not first in any system.
