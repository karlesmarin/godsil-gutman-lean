...
double cover of a smaller code's Tanner graph inherits its short-cycle profile through the classical
...
$A=x^3+y+y^2$, $B=y^3+x+x^2$, and $H_X=[A\mid B]$ (here $A,B$ denote the BB defining
polynomials, not the adjacency and non-backtracking operators of Section~\ref{sec:method}; the
$H_X$ and $H_Z$ Tanner graphs are isomorphic). The census (Table~\ref{tab:quantum}) finds girth~6 and $c_6=144$ shortest cycles,
...
The author declares no conflict of interest. This work received no funding.
