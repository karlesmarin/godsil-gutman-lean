...
handful of certified lemmas that fill missing pieces of Mathlib's linear-algebra ecosystem---Jacobi's
algebraic formula, the matrix-trace Newton identity, the resolvent series, and the directed walk
count.
\end{abstract}
...
limits are stated as plainly as the results.\footnote{\emph{Reproducibility.} All Lean sources are
public at \url{https://github.com/karlesmarin/godsil-gutman-lean} (commit \texttt{67d7d0a}), built
against Lean \texttt{v4.30.0-rc2} and the pinned Mathlib commit \texttt{701fb6e9c3}. Reproduce with
\texttt{lake exe cache get \&\& lake build}; check the axiom footprint of any result with
\lean{\#print axioms}. The numerical cross-checks of Table~\ref{tab:checks} are in
\texttt{verify\_jacobi\_newton.py}.}
...
$\adj M$ is the adjugate (the transpose of the cofactor matrix)~\cite{HornJohnson}. It is the matrix
calculus~\cite{MagnusNeudecker} behind Liouville's formula for the Wronskian of a linear ODE, behind
the Faddeev--LeVerrier algorithm that computes a characteristic polynomial from traces~\cite{Hou1998},
and behind every first-order perturbation of an eigenvalue. It is also, in its \emph{algebraic} form---over an arbitrary commutative ring, with the
...
The second is \emph{Newton's identities}~\cite{Macdonald}. Newton related the power sums
$p_k=\sum_i\lambda_i^k$ of a collection of numbers to their elementary symmetric functions
$e_k$---equivalently, the coefficients
...
This paper formalizes that chain---Jacobi, then Newton, then the Ihara counts---in Lean~4~\cite{LeanFour}, and is
...
\paragraph{Why this is useful, plainly.}
``Entry of a matrix power = number of walks'' is the workhorse of spectral graph theory~\cite{BrouwerHaemers}: mixing
...
\paragraph{Why this is useful, plainly.}
Non-backtracking counts and spectra underpin sharp epidemic and percolation
thresholds~\cite{KarrerNewmanZdeborova2014}, spectral community detection~\cite{Krzakala2013},
and---through the Ihara analogue of the Riemann hypothesis---the very definition of a Ramanujan graph
as an optimal expander~\cite{LPS1988,HLW2006}. Theorem~\ref{thm:ihara} certifies the bridge between
...
\toprule
\rowcolor{ggteal}
{\color{white}\textbf{Identity}} & {\color{white}\textbf{Instance}} & {\color{white}\textbf{Result}}\\
\midrule
...
(N1) the first machine-checked proof I am aware of, in any proof assistant---I searched Mathlib,
the Isabelle/HOL \emph{Archive of Formal Proofs}, Coq/Rocq's opam and \emph{coqdoc} indices,
HOL~Light, and Mizar's MML---of Jacobi's \emph{algebraic} formula $(\det M)'=\tr(\adj M\cdot M')$; (N2) the \emph{matrix-trace} form of
...
I carried out this development using an AI system as a research instrument. I set the
targets---which identities to formalize, the proof strategy for each, the boundary to claim and the
boundary to refuse---and the assistant proposed candidate definitions, lemmas and tactics, which a
build-as-oracle loop then accepted or rejected against the kernel. The mathematical decisions, the
scope, and every claim in this paper are mine and are my responsibility; the careful accounting that
results---each statement sized to exactly what was proved, the rooted-versus-geodesic distinction
drawn as sharply as the theorems---is part of what I hope the work shows: that AI-assisted
formalization, directed and audited by a human, can be held to a \emph{higher} standard of honesty
than informal mathematics, precisely because the proof assistant, not the assistant, has the last
word.
...
The mathematics is classical: Jacobi's formula and Liouville's~\cite{HornJohnson,MagnusNeudecker},
Newton's identities~\cite{Macdonald} and the Faddeev--LeVerrier algorithm built on them~\cite{Hou1998},
the walk-counting reading of matrix powers~\cite{BrouwerHaemers}, and---for the
...
Bass's formula~\cite{IharaBass}. After searching Mathlib, the Isabelle/HOL \emph{Archive of Formal
Proofs}, Coq/Rocq, HOL~Light, and Mizar, I am not aware of a prior machine-checked proof of
Jacobi's algebraic formula or of the matrix-trace Newton identity in any of them; I state this as a
search result, not a theorem, and would welcome a pointer to anything I missed.
...
\end{center}
{\sloppy in \lean{Ihara/TraceFormula.lean} (Jacobi, Newton, the Ihara composition) and
...
\bibitem{BrouwerHaemers} A.~E.~Brouwer and W.~H.~Haemers, \emph{Spectra of Graphs}, Universitext,
  Springer, 2012.
\bibitem{HLW2006} S.~Hoory, N.~Linial, and A.~Wigderson, \emph{Expander graphs and their
  applications}, Bull. Amer. Math. Soc. \textbf{43} (2006), 439--561.
...
\bibitem{KarrerNewmanZdeborova2014} B.~Karrer, M.~E.~J.~Newman, and L.~Zdeborov\'a, \emph{Percolation
  on sparse networks}, Phys. Rev. Lett. \textbf{113} (2014), 208702.
\bibitem{Krzakala2013} F.~Krzakala, C.~Moore, E.~Mossel, J.~Neeman, A.~Sly, L.~Zdeborov\'a, and
  P.~Zhang, \emph{Spectral redemption in clustering sparse networks}, Proc. Natl. Acad. Sci. USA
  \textbf{110} (2013), 20935--20940.
\bibitem{LeanFour} L.~de~Moura and S.~Ullrich, \emph{The Lean~4 theorem prover and programming
  language}, in \emph{Automated Deduction---CADE 28}, LNCS \textbf{12699}, Springer, 2021, 625--635.
\bibitem{LPS1988} A.~Lubotzky, R.~Phillips, and P.~Sarnak, \emph{Ramanujan graphs}, Combinatorica
  \textbf{8} (1988), 261--277.
\bibitem{Macdonald} I.~G.~Macdonald, \emph{Symmetric Functions and Hall Polynomials}, 2nd ed., Oxford
  University Press, 1995.
\bibitem{MagnusNeudecker} J.~R.~Magnus and H.~Neudecker, \emph{Matrix Differential Calculus with
  Applications in Statistics and Econometrics}, 3rd ed., Wiley, 2019.
